...
if ($_FILES['arquivo']['name']!=''){ //verifica se houve tentativa de subir a imagem
	...
	// Primeiro verifica se deve trocar o nome do arquivo
		...
		if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
			...
			$query ="insert into tb_planos (titulo, descricao, foto, para, datareg, userreg) values ('".$_POST['titulo']."', '".$_POST['descricao']."', '".$nome_final."', '".$_POST['para']."', '".date('Y-m-d H:i:s')."', '".$nomex."')";
			
			
			if (mysqli_query($conn, $query)) {
				echo "<script>alert('Plano inserido com sucesso!');</script>";
	...
	$query ="insert into tb_planos (titulo, descricao, para, datareg, userreg) values ('".$_POST['titulo']."', '".$_POST['descricao']."', '".$_POST['para']."', '".date('Y-m-d H:i:s')."', '".$nomex."')";
